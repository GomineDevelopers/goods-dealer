<template>
  <div class="buy">
    ...
    <div class="goods">
      <van-checkbox-group v-model="selectedGoods" @change="changeSelected">
        <div class="goods-check" v-for="(item,index) in orderList" :key="item.id">
          ...
          <van-swipe-cell :right-width="65">
            ...
            <span slot="right" @click="delGoods(item.id)">删除</span>
          </van-swipe-cell>
        </div>
      </van-checkbox-group>
    </div>
    ...
    methods: {
      getOrderList() {
        ...
        self.$http.get('https://icampaign.com.cn/gomineWechat/app/index.php', {
          ...
          self.selectedGoods = response.data.result.goods;
      ...
      decrease(index) {
        if (this.orderList[index].total > 1) {
          ...
          this.totalPrice = this.totalPrice - this.orderList[index].marketprice*100 * this.orderList[index].total;
        } else {
          this.$toast('不能再少了');
        }
      },
      add(index) {
        this.orderList[index].total++;
        this.totalPrice = this.totalPrice + this.orderList[index].marketprice*100 * (this.orderList[index].total-1);
      },
      onSubmit() {
        let vm = this;
        let postData = {};
      ...
      changeSelected(arr){
        ...
        for(let i =0;i<arr.length;i++){
          for(let j =0;j<vm.orderList.length;j++){
            if(arr[i].id === vm.orderList[j].id){

              vm.totalPrice = vm.totalPrice + vm.orderList[j].marketprice*100 *vm.orderList[j].total;
            }
          }
        }
      }
